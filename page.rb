...
page_senators = agent.get(conf.alternative_current_senate_members_url)
...
x.publicwhip do
  ...
  page.links[19..-4].each do |link|
	lastname = link.text.split(',')[0]
	firstname = link.text.split(',')[1]
	firstname =~ /\ *(the\ hon\.|the\ hon|miss|mrs|mr|dr|ms)?\ *(.*)/i
	firstname = $2
    name = Name.last_title_first("#{lastname},  #{firstname}")
  ...
  #senators
  page_senators.links[25..-4].each do |link|
	#puts "link.text = #{link.text}"
	lastname = link.text.split(',')[0]
	firstname = link.text.split(',')[1].split('-')[0] #removes ' - the senator for (state)
	firstname =~ /\ *(the\ hon\.|miss|mrs|mr|dr|ms)?\ *(.*)/i
	firstname = $2
    name = Name.last_title_first("#{lastname},  #{firstname}")
    person = people.find_person_by_name_current_on_date(name, Date.today)
    if person
      sub_page = agent.click(link)
	  puts "Senator #{lastname}'s personal home page"
      home_page_tag = sub_page.links.find{|l| l.text == "Senator #{lastname}'s personal home page"}
      
      params = {:id => person.id, :mp_contactdetails => sub_page.uri}
      params[:mp_website] = home_page_tag.uri if home_page_tag
      x.personinfo(params)
    else
      puts "WARNING: Could not find senator with name #{name.full_name}"
    end
  end
